...
func TestApplyRun(t *testing.T) {
	...
	testCases := []testCase{
		...
		},
		{
			...
			input: &applyRunInput{ID: "r1"},
			mockSetup: func(m *runMocks) {
				m.acl.On("Authorize", mock.Anything, mock.Anything, "r1", trn.TypeRun).Return(status.Error(codes.PermissionDenied, "access denied"))
			},
			expectError: true,
		},
		{
			...
			mockSetup: func(m *runMocks) {
				m.acl.On("Authorize", mock.Anything, mock.Anything, "nonexistent", trn.TypeRun).Return(nil)
