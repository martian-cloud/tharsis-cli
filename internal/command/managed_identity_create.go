...
func (c *managedIdentityCreateCommand) validate() error {
	if c.name != nil && len(c.arguments) > 0 {
		return errors.New("name must be provided as either an argument or -name flag, not both")
	}

	if c.name == nil && len(c.arguments) == 0 {
		return errors.New("name is required either as an argument or -name flag")
	}

	if c.groupID == nil {
		return errors.New("group id or path is required")
	}
	...
	case pb.ManagedIdentityType_aws_federated.String():
		if c.awsFederatedRole == nil {
			return fmt.Errorf("aws federated role is required for type %s", *c.identityType)
		}
	case pb.ManagedIdentityType_azure_federated.String():
		if c.azureFederatedClientID == nil {
			return fmt.Errorf("azure federated client id is required for type %s", *c.identityType)
		}

		if c.azureFederatedTenantID == nil {
			return fmt.Errorf("azure federated tenant id is required for type %s", *c.identityType)
		}
	case pb.ManagedIdentityType_tharsis_federated.String():
		if c.tharsisFederatedServiceAccountPath == nil {
			return fmt.Errorf("tharsis federated service account path is required for type %s", *c.identityType)
		}
	case pb.ManagedIdentityType_kubernetes_federated.String():
		if c.kubernetesFederatedAudience == nil {
			return fmt.Errorf("kubernetes federated audience is required for type %s", *c.identityType)
		}
	}
