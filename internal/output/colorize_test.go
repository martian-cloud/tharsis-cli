...
func TestColorize(t *testing.T) {
	...
	t.Run("highlights required marker on flag lines", func(t *testing.T) {
		result := Colorize("  -group-path *\n      Path of group.", "")

		assert.Contains(t, result, color.New(color.FgRed).Sprint("*"))
	})

	t.Run("highlights deprecated marker on flag lines", func(t *testing.T) {
		result := Colorize("  -old !\n      Old flag.", "")

		assert.Contains(t, result, color.New(color.FgYellow).Sprint("!"))
	})

	t.Run("highlights repeatable marker on flag lines", func(t *testing.T) {
		result := Colorize("  -tag ...\n      A tag value.", "")

		assert.Contains(t, result, color.New(color.FgHiGreen).Sprint("..."))
	})

	t.Run("does not highlight markers in description text", func(t *testing.T) {
