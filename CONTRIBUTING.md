...
> **Local testing only (temporary):** a `make prerelease` target exists to mirror this same flow from your local machine — useful when the CI job can't be exercised pre-merge because `RELEASE_TOKEN` is a protected variable. Run `GITLAB_TOKEN=<your-pat> VERSION=v0.36.0-alpha.2 ALLOW_NON_MAIN=1 make prerelease`. This target will be removed once the CI prerelease path is validated end-to-end.
